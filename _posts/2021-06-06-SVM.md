...
`Once we minimize the $\|\|W\|\|$, we maximize the margin $m$ and hence we would find our optimal hyperplane lying in middle fof $H_o$ and $H_1$.`
To make the problem easier when taking the gradients, we will try to minimize the following form:

$$
min \space \frac{1}{2}\|W\|^{2} = min \space \frac{1}{2}W^{T}W \space s.t. \space y_{i}(W.X+b) \geq 1
$$

#### Linear Inseparability
All the examples we have seen until so far were linear separable i.e., the data points could be classified with a straight line. In real world, dataset is not easy to classify with just a straight line and are often linear inseparable. Look at one of the examples of linear inseparable data below:
