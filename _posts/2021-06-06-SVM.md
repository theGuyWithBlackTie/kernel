...
$$
D = {(X_i, y_i) \| X_i \belongs R^p, y_i \belongs {-1, 1}}_{i=1}^{n}
$$
...
So let's assume dataset $D$ is linearly separable and would want to find two hyperplanes with no points between them without visualisation. 
