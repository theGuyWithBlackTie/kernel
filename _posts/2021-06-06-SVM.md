...
$$
y_i \times (W.X_i + b) \geq y_i(-1) \Longrightarrow y_i \times (W.X_i + b) \geq 1 for\space X_i \space having \space class \space -1 \space \space \space \space (3)
$$

In equation (1), multiply both sides with $y_i$ and as $y_i =1$, sign of inequation doesn't change

$$
y_i \times (W.X_i + b) \geq 1 for\space X_i \space having \space class \space 1 \space \space \space \space (4)
$$

Nw we can combine equations (3) and (4) and have one equation for both the constraints

$$
y_i \times (W.X_i + b) \geq 1 for \space all \space 1 \leq i \leqn n
$$
