...
### Model Optimization
Now that loss functions for **generator** and **discriminator** has been defined, it's time to leverage mathematics to solve the optimization problem, i.e. finding the parameters for the **generator** and **discriminator** such that the loss functions are optimized.
...
$$
V(G, D^{*}) = -log4 + D_{KL}(P_data \| \frac{p_{data}+p_{g}}{2}) + D_{KL}(p_{g} \| \frac{p_{g}+p_{data}}{2}) \tag{11}
$$

Jensen-Shannon divergence is defined as:

$$
J(P,Q) = \frac{1}{2}(D(P \| E)) + D(Q \| R)
$$
...
The conclusinon of this analysis is simple: the goal of training the generator, which is to minimize the value function $V(G,D)$ is equal to the JS divergence between the distribution of the data and the distribution of generated examples to be as small as possible. This conclusion aligns with the intuition that generator is being able to learn the underlying distribution of the data from sampled training examples. In other words, $p_{g}$ and $p_{data}$ should be close to each other as possible. The optimal generator $G$ is thus the one that which is able to mimic $p_{data}$ to model a compelling distribution $p_{g}$.
